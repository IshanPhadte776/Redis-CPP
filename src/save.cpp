...
// using RedisData = std::variant<std::string, std::vector<std::string>>;
...
//             else if (command.bulkString == "RPUSH" && request.elements.size() > 2) {
//                 std::string key = request.elements[1].bulkString;
//                 std::lock_guard<std::mutex> lock(store_mutex);

//                     // 1. Get or Create the Node
//                 if (kv_store.find(key) == kv_store.end()) {
//                     kv_store[key] = Node{ std::vector<std::string>{}, {}, false };
//                 }

//                 // 2. Ensure the key is actually a List and not a String (from a previous SET)
//                 if (!std::holds_alternative<std::vector<std::string>>(kv_store[key].data)) {
//                     const char* err = "-WRONGTYPE Operation against a key holding the wrong kind of value\r\n";
//                     send(client_fd, err, strlen(err), 0);
//                 } else {
//                     // 3. Append all values provided in the command
//                     auto& list = std::get<std::vector<std::string>>(kv_store[key].data);
//                     for (size_t i = 2; i < request.elements.size(); ++i) {
//                         list.push_back(request.elements[i].bulkString);
//                     }

//                     // 4. Return the new length of the list (integer response)
